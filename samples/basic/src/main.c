...
void HAL_task(void)
{
    u8_t data = 1;
    while (1) {
        zt_channel_set(ZT_SENSOR_VAL_CHANNEL, (u8_t *) &data, sizeof(data));
        k_sleep(K_SECONDS(3));
...
void CORE_task(void)
{
    ...
    while (1) {
        ...
        case ZT_SENSOR_VAL_CHANNEL:
            zt_channel_get(zt_core_evt_id, (u8_t *) &data, sizeof(data));
            printk("[CORE_service_callback] Getting data value %02X\n", data);
            for (u8_t i = 1; i <= data; ++i) {
                result = result * power;
            }
            zt_channel_set(ZT_POWER_VAL_CHANNEL, (u8_t *) &result, sizeof(result));
            break;
        default:
...
void APP_task(void)
{
    ...
    zt_channel_get(ZT_FIRMWARE_VERSION_CHANNEL, fv, sizeof(fv));
    ...
    while (1) {
        ...
        case ZT_POWER_VAL_CHANNEL:
            zt_channel_get(zt_app_evt_id, (u8_t *) &data, sizeof(data));
