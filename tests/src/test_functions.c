...
void CORE_task(void)
{
    ...
    while (1) {
        ...
        case ZT_POWER_VAL_CHANNEL:
            error = zt_chan_raw_read(ZT_POWER_VAL_CHANNEL, (u8_t *) &data, sizeof(data));
...
void HAL_task(void)
{
    ...
    error = zt_chan_raw_read(ZT_CHANNEL_COUNT, &data, sizeof(data));
    ...
    error = zt_chan_raw_read(ZT_DATA_VAL_CHANNEL, &data, 40);
    zassert_equal(error, -EINVAL,
                  "Get function either isn't checking data size or is checking wrong, "
                  "error code: %d!\n",
                  error);
    zt_data_t *g1 = ZT_DATA_U64(0);
    error         = zt_chan_read(ZT_DATA_VAL_CHANNEL, g1);
    zassert_equal(error, -EINVAL,
                  "Get function either isn't checking data size or is checking wrong, "
                  "error code: %d!\n",
                  error);

    error = zt_chan_raw_read(ZT_DATA_VAL_CHANNEL, NULL, sizeof(data));
    ...
    error = zt_chan_raw_read(ZT_DATA_VAL_CHANNEL, &data, sizeof(data));
    ...
    error         = zt_chan_read(ZT_DATA_VAL_CHANNEL, g2);
    zassert_false(error, "Error executing a valid publish call!\n");
    ...
    error = zt_chan_raw_pub(ZT_CHANNEL_COUNT, &data, sizeof(data));
    zassert_equal(error, -ENODATA,
    ...
    power_val = 50;
    error = zt_chan_raw_pub(ZT_POWER_VAL_CHANNEL, (u8_t *) &power_val, sizeof(power_val));
    ...
    zt_chan_read(ZT_POWER_VAL_CHANNEL, d3);
    zassert_equal(d3->u16.value, 78, "POWER_VAL (%d) isn't the expected value 78",
                  d3->u16.value);
    d3->u16.value = 1;
    error         = zt_chan_pub(ZT_POWER_VAL_CHANNEL, d3);
    zassert_equal(error, 0, "Error executing a valid publish call!\n");
