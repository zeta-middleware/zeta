...
void test_channels_name(void)
{
    ...
    error = 1;
    name  = zeta_channel_name(ZETA_DATA_VAL_CHANNEL, &error);
    zassert_equal(error, 0, "[%s] zeta_channel_name is not setting error to 0 in a valid call!\n",
                  __FUNCTION__);
    zassert_true(!strcmp(name, "DATA_VAL"), "[%s] channel #%d was created with a wrong name: %s\n",
                 __FUNCTION__, ZETA_DATA_VAL_CHANNEL, name);
...
void test_channels_size(void)
{
    ...
    sz = zeta_channel_size(ZETA_DATA_VAL_CHANNEL, &error);
    zassert_equal(error, 0, "[%s] zeta_channel_size is setting error to %d in a valid call!\n",
                  __FUNCTION__, error);
...
void test_channels_routine(void)
{
    ...
    /*               "The last thread that get DATA_VAL channel should be the HAL thread!\n"); */
}

void test_main(void)
{
    ztest_test_suite(ZETA_CHECK_CHANNELS_GENERATION, ztest_unit_test(test_channels_name),
                     ztest_unit_test(test_channels_size), ztest_unit_test(test_channels_routine));
